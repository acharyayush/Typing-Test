...
languages.forEach((lan) => {
  lan.addEventListener("click", () => {
    ...
    lan == languages[0]
      ...
      : (paragraphs = nepaliUnicodeParagraphs);
    reset();
  });
});
//switch Time
...
inputText.addEventListener("keydown", (e) => preventKeysAndCombinations(e));
menuIcon.addEventListener("click", () => {
  menusContainer.classList.toggle("menusContainerActive");
});
//Functions
function preventKeysAndCombinations(e) {
  /*
  Forbidden keys: arrow keys
  Forbidden Combinations: Ctrl + A, Ctrl + backspace, Ctrl + Z, Ctrl + Y
  */
  let key = e.key.toLowerCase();
  let { ctrlKey, metaKey } = e;
  //for arrowLeft, arrowRight, arrowUp and arrowDown
  if (key.includes("arrow")) e.preventDefault();
  //for ctrl+A
  else if ((ctrlKey || metaKey) && key === "a") e.preventDefault();
  else if ((ctrlKey || metaKey) && key === "backspace") e.preventDefault();
  else if ((ctrlKey || metaKey) && key === "z") e.preventDefault();
  else if ((ctrlKey || metaKey) && key === "y") e.preventDefault();
}
